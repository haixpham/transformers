...
class PreTrainedModel(nn.Module, ModuleUtilsMixin, GenerationMixin, PushToHubMixin, PeftAdapterMixin):
    ...
    def _tie_or_clone_weights(self, output_embeddings, input_embeddings):
        ...
        if getattr(output_embeddings, "bias", None) is not None:
            if is_deepspeed_zero3_enabled():
                ...
                params = [output_embeddings.weight, output_embeddings.bias]
    ...
    def _get_resized_embeddings(
        ...
        if is_deepspeed_zero3_enabled():
            ...
            with ContextManagers([deepspeed.zero.Init(config_dict_or_path=deepspeed_config())]):
    ...
    def _load_pretrained_model(
        ...
        if is_deepspeed_zero3_enabled():
            model_state_dict = _load_deepspeed_wrapped_model_state_dict(model)
        else:
            ...
            if ignore_mismatched_sizes:
                for checkpoint_key in loaded_keys:
                    ...
                    if (
                        ...
                        and ((state_dict[checkpoint_key].shape != model_state_dict[model_key].shape and not hasattr(model_state_dict[model_key], "ds_shape"))
                        # new condition
                        or (hasattr(model_state_dict[model_key], "ds_shape") and state_dict[checkpoint_key].shape != model_state_dict[model_key].ds_shape))
                    ):
...
def _load_deepspeed_wrapped_model_state_dict(model):
